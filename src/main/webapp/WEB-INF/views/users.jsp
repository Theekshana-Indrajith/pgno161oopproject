...
<html>
<head>
    <title>User Management - CinemaScope</title>
...
<section class="reservation-section">
    <div class="page-header">
        ...
        <p>Manage user roles and view user details</p>
    </div>
            ...
            <c:forEach var="user" items="${users}">
                <div class="user-card">
                    ...
                    <div class="d-flex gap-2 justify-content-center flex-wrap">
                        <a href="/users/details/${fn:escapeXml(user.username)}" class="btn btn-info">View Details</a>
                        <c:if test="${user.role == 'USER'}">
                            <form action="/users/promote" method="post">
                                <input type="hidden" name="username" value="${fn:escapeXml(user.username)}">
                                <button type="submit" class="btn btn-success" onclick="return confirm('Are you sure you want to promote ${fn:escapeXml(user.username)} to ADMIN?');">Promote to Admin</button>
                            </form>
                        </c:if>
                        <c:if test="${user.role == 'ADMIN'}">
                            <form action="/users/demote" method="post">
                                <input type="hidden" name="username" value="${fn:escapeXml(user.username)}">
                                <button type="submit" class="btn btn-warning" onclick="return confirm('Are you sure you want to demote ${fn:escapeXml(user.username)} to USER?');">Demote to User</button>
                            </form>
                        </c:if>
                    </div>
                </div>
